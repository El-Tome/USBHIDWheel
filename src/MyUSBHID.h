...
class MyUSBHID {
public:
  MyUSBHID(hid_interface_protocol_enum_t itf_protocol = HID_ITF_PROTOCOL_NONE);
  void begin(void);
  void end(void);
  bool ready(void);
  ...
  static bool addDevice(MyUSBHIDDevice *device, uint16_t descriptor_len);
};
