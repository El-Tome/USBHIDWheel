...
static bool tinyusb_enable_hid_device(uint16_t descriptor_len, MyUSBHIDDevice *device) {
...
MyUSBHIDDevice *tinyusb_get_device_by_report_id(uint8_t report_id) {
...
static uint16_t tinyusb_on_get_feature(uint8_t report_id, uint8_t *buffer, uint16_t reqlen) {
  MyUSBHIDDevice *device = tinyusb_get_device_by_report_id(report_id);
  if (device) {
...
MyUSBHID::MyUSBHID(hid_interface_protocol_enum_t itf_protocol) {
...
bool MyUSBHID::SendReport(uint8_t id, const void *data, size_t len, uint32_t timeout_ms) {
...
bool MyUSBHID::addDevice(MyUSBHIDDevice *device, uint16_t descriptor_len) {
...
void MyUSBHID::onEvent(esp_event_handler_t callback) {
  onEvent(ARDUINO_USB_HID_ANY_EVENT, callback);
}
void MyUSBHID::onEvent(arduino_usb_hid_event_t event, esp_event_handler_t callback) {
